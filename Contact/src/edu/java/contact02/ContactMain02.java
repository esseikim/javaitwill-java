...
public class ContactMain02 {
	...
	private int count = 0; // 연락처 배열에 현재까지 저장된 연락처의 개수. 배열에 저장될 때마다 값 증가.(삭제 시 값 감소)
	...
	public static void main(String[] args) {
		...
		// -> static에서 static이 아닌 메서드들을 사용하기 위해서 참조 변수를 생성.

		boolean run = true;

		while (run) {
			int n = app.showMainMenu();
			Menu me = Menu.getValue(n);
			switch (me) {
			case QUIT: // 프로그램 종료
				run = false;
				break;
	...
	public int inputIndex() {
		while (true) {   // 몇번만에 입력 끝날지 모름. 
			try {
				int n = Integer.parseInt(scanner.nextLine());
				return n;  // (1) 값을 메서드 호출한 곳에 반환 (2) 메서드 종료  <- return을 만나는 순간 메서드 끝남. 
			}
