...
public class ContactUpdateFrame extends JFrame {  
	...
	public static void showContactUPdateFrame(Component parent, int index, ContactMain06 ap) {  
		// this.parent = parent; 바로 접근 불가. 
		EventQueue.invokeLater(new Runnable() {  
			public void run() {
				try {
					ContactUpdateFrame frame = new ContactUpdateFrame(parent, index, ap); 
	 ...
	 * Create the frame.
	 */
	public ContactUpdateFrame(Component parent, int index, ContactMain06 app) { // 생성자 
		this.parent = parent; 
		this.index = index;  // 필드에 선언해야 에러 안남. 
		this.app = app;
		initialize(); 
	...
	protected void UpdateContact() { 
		...
		if (confirm == JOptionPane.YES_OPTION) {
			dao.update(index, contact); // 다오에서 crate함. 리스트를 수정하고, 파일에 업데이트
