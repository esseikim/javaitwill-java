...
public interface ContactDao {  // public static final 상수, 추상메서드, 아닌 경우 static, default를 붙여줘야
	...
	/**
	 * 새 연락처 저장 기능. 전달받은 연락처 객체를 리스트에 저장.
	...
	/**
	 *  연락처 전체 목록 보기 기능. 저장된 연락처 개수크기의 연락처 리스트를 리턴. 
	 *  (무엇을 리턴해줘야 메인은 그걸 리턴받아서 프린트 할 수 있을까?) 
	 *  @return 연락처 리스트(Contact[]).  보여주기만 하면 됨. 전체를. 파라미터 불필요
	 */
	List<Contact> read();
	...
	// body는 이렇게 만들어주면돼~ 인터페이스 (사용법)  -사용법과 규격의 역할이 인터페이스
